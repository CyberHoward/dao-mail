...
fn count() -> anyhow::Result<()> {
    ...
    // Set up the contract
    let contract = DkimAuthI::setup(mock.clone(), mock.sender().clone())?;
    ...
    // Get the count.
    use dkim_auth::msg::QueryMsgFns;
    let count1 = contract.get_count()?;

    // or query it manually
    let count2: CounterResponse = contract.query(&QueryMsg::GetCount {})?;
    assert_eq!(count1.count, count2.count);

    // Or get it manually from the chain
    let count3: CounterResponse = mock.query(&QueryMsg::GetCount {}, &contract.address()?)?;
    assert_eq!(count1.count, count3.count);

    // Check the count
    assert_eq!(count1.count, 2);
    // Reset
    use dkim_auth::msg::ExecuteMsgFns;
    contract.reset(0)?;
