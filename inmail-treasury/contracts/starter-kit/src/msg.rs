...
impl EmailAuthDetails {
    pub fn get_sender(&self) -> Result<EmailAddress, AuthenticatorError> {
        if let Some(message) = mail_parser::MessageParser::default().parse(&self.headers.clone()) {
            // TODO HACKATHON: fix this thing
            if let Some(from) = message.from() {
                from.first()
                    .ok_or(AuthenticatorError::NoSenderFound(self.headers.clone()))?
            } else {
                println!("No sender found in headers: {}", self.headers);
                return Err(AuthenticatorError::NoSenderFound(self.headers.clone()));
            }
        } else {
            return Err(AuthenticatorError::FailedToParseHeaders(
                self.headers.clone(),
            ));
        };
